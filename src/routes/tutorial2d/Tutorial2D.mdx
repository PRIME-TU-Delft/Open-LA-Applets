...
by passing a `title` prop, which has to be `LocalizedString`. We will also import a `Vector2D` component to show a simple arrow on the canvas. You can
...
<Canvas2D draggables={[direction1]} title={{"en": "Complex numbers addition", "nl": "Optellen van complexe getallen"}} >
  <Vector2D
...
<Canvas2D {controls} draggables="{[direction1]}" title={{"en": "Complex numbers addition", "nl": "Optellen van complexe getallen"}}>
  <!-- Components -->
</Canvas2D>
```
