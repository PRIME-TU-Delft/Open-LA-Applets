...
- `domain` (optional): `{ xMin?: number; xMax?: number }`, can be set to `undefined` to cover the whole grid.
- `isDashed` (optional): `true` or (by default ) `false`.
- `shape` (optional): `circle`, `square` or `triangle`, used for the points (gaps, included points) and legend item shapes, uses `circle` by default.

Useful chaining methods:

```ts
new FunctionFragment('\\frac{{x+1}^2}{x+1}', PrimeColor.blue, { xMax: 3 }, true)
  .addGaps(new Vector2(-1, 0))
  .addIncludedPoints(new Vector2(3, 4))
  .withIntegral({ xLeft: -4, xRight: -1.25, fillStyle: 'full' })
  .withLegend("Function f", FillType.Border);
```
...
- `.withLegend(text: string, fill?: FillType)`: adds legend item for this function, with the specified text and fill type (default: `FillType.Full`, `FillType.Dashed`, `FillType.Border`).
...
new FunctionFragment((x: number) => -x, PrimeColor.raspberry, undefined, false, 'square').withLegend(
  'Some function f(x)',
  FillType.Dashed
);

// 7) Function with only legend defined
new FunctionFragment((x: number) => x ** 2 - 3, PrimeColor.raspberry).withLegend(
...
- Optionally set `initialViewBox` or `cameraPosition`, `cameraZoom` to control the initial camera framing.
