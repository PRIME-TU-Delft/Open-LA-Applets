...
If you want a custom starting camera, define a `ViewBox` in the `camera settings` part. If not, then remove the `camera settings` lines.
...
## Optional: set an initial view with `ViewBox`
