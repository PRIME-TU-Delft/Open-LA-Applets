<script lang="ts">
  ...
  const Integrand = (x: number) => 2 * Math.exp(-1 * (-0.5 * x) ** 2);
  const Integral = (x: number) => 2 * Math.sqrt(Math.PI) * calculateErf(0.5 * x);
  const appletObjects: AppletObject[] = [
    new FunctionFragment(Integrand, PrimeColor.raspberry, {
      legendText: 'f(t)=2e^{-\\frac{1}{4}t^2}',
      domain: { xMin: 0 },
      width: 0.06,
      integral: {
        xLeft: 0,
        xRight: 0,
        legendText: 'F(x)=\\displaystyle\\int_0^x2e^{-\\frac{1}{4}t^2}\\,dt',
