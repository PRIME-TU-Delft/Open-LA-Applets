<script lang="ts">
  ...
  const der2 = (x: number) => (-2 / 9) * Math.pow(x, -5 / 3);
  ...
  const LegendTs = $derived.by(() => {
    ...
    const invder2 = 9 * Math.pow(fx, 5);
    const legendT1 = new LegendItem(
      'T_1(x)=\\$2+\\$3 (x-\\$1)',
      PrimeColor.orange + PrimeColor.opacity(0.5),
      'square'
    )
    ...
    const legendT2 = new LegendItem(
      'T_2(x)=\\$2+\\$3 (x-\\$1) - \\$4 (x-\\$1)^2',
      PrimeColor.raspberry,
      'triangle'
    )
      .addAutoParam(x.toFixed(3).replace(/\.?0+$/, ''), PrimeColor.darkGreen)
      .addAutoParam(fx.toFixed(1).replace(/\.?0+$/, ''), PrimeColor.darkGreen)
      .addAutoParam('\\frac{1}{' + invder.toFixed(2).replace(/\.?0+$/, '') + '}', PrimeColor.orange)
      .addAutoParam(
        '\\frac{1}{' + invder2.toFixed(5).replace(/\.?0+$/, '') + '}',
        PrimeColor.raspberry
      );
    return [legendT1, legendT2];
  });
</script>
...
>
  ...
  {@const fppx0 = der2(x0)}
  <ExplicitFunction2D
    func={(x: number) => fx0 + fpx0 * (x - x0) + 0.5 * fppx0 * (x - x0) ** 2}
