<script lang="ts">
  ...
  function snapToFuncA(position: Vector2): Vector2 {
    ...
    // Keep point B on the same vertical line through A without changing the angle.
    draggablePoint[1].value = new Vector2(x, x * Math.tan(angle));

    return new Vector2(x, 0);
  }
  function snapToFuncB(position: Vector2): Vector2 {
    const x = draggablePoint[0].position.x;
    // do not allow 0 or less for u
    const y = Math.max(0.01, position.y);
  ...
  const draggablePoint = [
    ...
    new Draggable(new Vector2(A, B), PrimeColor.darkGreen, undefined, snapToFuncB)
  ];
</script>
...
>
  ...
  <Point2D position={new Vector2(draggablePoint[1].position.x, Dy)} color={PrimeColor.raspberry} />
  <Latex2D
    latex="C"
  ...
  <Latex2D
    ...
    position={new Vector2(draggablePoint[1].position.x + 0.1, Dy)}
    alignX="left"
    alignY="bottom"
  />
  <Latex2D
