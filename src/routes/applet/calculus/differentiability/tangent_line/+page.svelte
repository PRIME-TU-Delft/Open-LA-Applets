<script lang="ts">
  ...
  const legendItems = $derived([
    ...
    new LegendItem(legendB(), PrimeColor.raspberry)
  ]);
  ...
  function Direction() {
    const dx = controls[0];
    const dy = baseFunction(draggablePoint[0].position.x + dx) - draggablePoint[0].position.y;
...
>
  ...
  {#if controls[0] === 0}
    <InfiniteLine2D
      origin={draggablePoint[0].position}
      direction={new Vector2(1, (3 / 8) * draggablePoint[0].position.x ** 2)}
      color={PrimeColor.darkGreen}
      width={0.08}
    />
  {/if}
  <Point2D
    position={new Vector2(
      draggablePoint[0].position.x + controls[0],
      baseFunction(draggablePoint[0].position.x + controls[0])
    )}
