...
>
  ...
  />
  <Latex2D
    latex={String.raw`\arctan\left(-2\right)+\pi\approx` + theta.toFixed(2)}
    color={PrimeColor.orange}
  ...
  <Line2D
    start={draggablePoint[0].position.clone().multiplyScalar(-1)}
    end={new Vector2(0, 0)}
  ...
  />
  <Latex2D
    latex={String.raw`\arctan\left(-2\right)\approx-` + theta.toFixed(2)}
    color={PrimeColor.pink}
    position={new Vector2(
      0.5 * r * Math.cos((theta - Math.PI) / 2),
      0.5 * r * Math.sin((theta - Math.PI) / 2)
    ).add(
      new Vector2(Math.cos((theta - Math.PI) / 2), Math.sin((theta - Math.PI) / 2)).multiplyScalar(
        0.5
      )
    )}
    alignX="left"
