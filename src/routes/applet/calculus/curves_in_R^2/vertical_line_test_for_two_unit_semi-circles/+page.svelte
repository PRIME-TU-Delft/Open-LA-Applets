<script lang="ts">
  ...
  function snapToCircle2(position: Vector2) {
    let x = position.x;
    let y = position.y;
    let theta = Math.atan2(y, x);
    theta = theta / Math.PI;
  ...
  const xD2 = $derived(draggables2[0].position.x);
  const xC2 = $derived(draggables2[1].position.x);
  const yC2 = $derived(draggables2[1].position.y);

  function ZeroFunc(x: number, y: number): number {
    return x ** 2 + y ** 2 - a ** 2;
  }
  const appletObjects: AppletObject[] = [
    new ImplicitFunctionFragment(ZeroFunc, PrimeColor.blue, {
      width: 0.08,
      range: { yMin: 0 },
      legendText: 'y=\\sqrt{1-x^2}'
    })
  ];
  const appletObjects2: AppletObject[] = [
    new ImplicitFunctionFragment(ZeroFunc, PrimeColor.red, {
      width: 0.08,
      range: { yMax: 0 },
      legendText: 'y=-\\sqrt{1-x^2}'
    })
...
>
  ...
  >
    ...
    {#if Math.abs(xD2) <= a}
      <Point2D
        color={PrimeColor.yellow}
        position={new Vector2(xD2, BottomFunction(xD2))}
        shape="square"
        radius={0.15}
      />
    {/if}
    <Line2D
      start={draggables2[1].position}
      end={new Vector2(xC2, 0)}
      isDashed={true}
      color={PrimeColor.cyan}
      width={0.05}
    />
    ...
    <Point2D
      color={PrimeColor.cyan}
      position={new Vector2(xC2, 0)}
      shape="triangle"
      radius={0.15}
    />
    <Point2D
      color={PrimeColor.green}
      position={new Vector2(0, yC2)}
      shape="triangle"
      radius={0.15}
    />
    <Latex2D
      latex="c"
      position={new Vector2(xC2, 0.15)}
      alignY={yC2 < 0 ? 'bottom' : 'top'}
      alignX="center"
      color={PrimeColor.cyan}
    />
    <Latex2D
      latex="d"
      position={new Vector2(xC2 > 0 ? -0.25 : 0.15, yC2)}
      alignX={xC2 < 0 ? 'left' : 'right'}
      alignY="center"
      color={PrimeColor.green}
    />
    <Latex2D
      latex="(c,d)"
      position={draggables2[1].position}
      alignX={xC2 > 0 ? 'left' : 'right'}
      alignY={yC2 > 0 ? 'bottom' : 'top'}
      color={PrimeColor.pink}
    />
