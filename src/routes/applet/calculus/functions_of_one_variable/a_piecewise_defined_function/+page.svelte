<script lang="ts">
  ...
  const appletObjects: AppletObject[] = [
    new FunctionFragment('2x+3', PrimeColor.blue, {
      domain: { xMax: -1 },
      legendText:
        'f(x)=\\left\\{\\begin{array}{lll}2x+3,\\qquad&\\text{if }&x\\leq -1,\\\\ 3,&\\text{if }&0<x\\leq 1,\\\\ 2-x^2,&\\text{if }&x>1.\\end{array}\\right.'
    }).addIncludedPoints(new Vector2(-1, 1)),
    new FunctionFragment('3', PrimeColor.blue, {
      domain: { xMin: 0, xMax: 1 }
    })
      .addGaps(new Vector2(0, 3))
      .addIncludedPoints(new Vector2(1, 3)),
    new FunctionFragment('2-x^2', PrimeColor.blue, {
      domain: { xMin: 1 }
    }).addGaps(new Vector2(1, 1))
  ];
</script>
