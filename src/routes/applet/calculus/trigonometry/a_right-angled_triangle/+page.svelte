...
<Canvas2D {initialViewBox} legendItems={getLegend(appletObjects)} axis={null} {scaleX} {scaleY}>
  ...
  <Latex2D
    latex="\theta"
    position={new Vector2(1.6, 0.5)}
    color={PrimeColor.orange}
    alignX="center"
    alignY="center"
  />
  <Latex2D
    latex={String.raw`\text{adj}`}
    position={new Vector2(2, -0.15)}
    alignX="center"
    alignY="top"
  />
  <Latex2D
    latex={String.raw`\text{opp}`}
    position={new Vector2(4.15, 1.5)}
    alignX="left"
    alignY="center"
  />
  <Latex2D
    latex={String.raw`\text{hyp}`}
    position={new Vector2(2.1, 1.6)}
    alignX="right"
    alignY="bottom"
  />
</Canvas2D>
