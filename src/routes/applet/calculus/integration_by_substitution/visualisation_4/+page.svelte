<script lang="ts">
  ...
  // APPLET OBJECTS
  // ##############
  const Transform = (x: number) => Math.asin(x);
  const Integrand = (x: number) => Math.sqrt(1 - x ** 2);
  const IntegrandU = (u: number) => Math.cos(u) ** 2;
  function SnapTopA(position: Vector2): Vector2 {
    // increments of 0.01, minimum -1, maximum right point of the INTERVAL (Math.E initially) minus 0.01
  ...
  function SnapTopB(position: Vector2): Vector2 {
    // increments of 0.01, maximum 1, minimum left point of the INTERVAL (1 initially) plus 0.01
    let x = Number(position.x.toFixed(2));
  ...
  const legendItems = $derived.by(() => {
    ...
    return [
      new LegendItem('u=\\arcsin(x)', PrimeColor.blue, 'square'),
      new LegendItem(
      ...
      new LegendItem(
        '\\int_{\\arcsin(\\$1)}^{\\arcsin(\\$2)}\\cos^2(t)\\,dt',
