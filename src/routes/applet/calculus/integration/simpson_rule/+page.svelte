<script lang="ts">
  ...
  const draggables = [
    new Draggable(new Vector2(8, 0), PrimeColor.orange, 'x_R', xrSnapFunc, undefined, 'bottom'),
    new Draggable(new Vector2(1, 0), PrimeColor.orange, 'x_L', xlSnapFunc, undefined, 'bottom')
  ];
  ...
  // interpolate quadratic through 3 points
  const quadraticFunc = $derived((x: number) => {
    const yL = func(xL);
    const yM = func(xM);
    const yR = func(xR);
  ...
  const formulas = $derived.by(() => [
    ...
    new Formula('\\frac{x_R - x_L}{6}(f(x_L) + 4f(x_M) + f(x_R)) = \\$1').addAutoParam(
      round(area),
      PrimeColor.orange
    )
  ]);
...
<Canvas2D
  ...
  title={$_('applets.calculus.integration.simpson_rule.title')}
>
  <ExplicitFunction2D
    {func}
