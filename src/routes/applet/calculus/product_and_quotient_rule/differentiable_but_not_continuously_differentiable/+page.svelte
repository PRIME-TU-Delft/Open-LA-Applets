<script lang="ts">
  ...
  initialViewBox = new ViewBox(
    new Vector2(-1, -1.4), // bottom-left
    new Vector2(1, 1.5), // top-right
    0 // margin
  );
  ...
  const appletObjects: AppletObject[] = [
    new FunctionFragment((x: number) => x ** 2 * Math.sin(1 / x), PrimeColor.blue, {
      legendText: 'f(x)'
    })
  ];
  const appletObjects2: AppletObject[] = [
    new FunctionFragment(
      (x: number) => -Math.cos(1 / x) + 2 * x * Math.sin(1 / x),
