<script>
  ...
  let d1 = new Draggable(new Vector2(1, 2), PrimeColor.blue, 'v_1', Draggable.snapToGrid);
  let d2 = new Draggable(new Vector2(2, 0), PrimeColor.darkGreen, 'v_2', Draggable.snapToGrid);
  ...
  const formulas = $derived.by(() => {
    const f1 = new Formula(
      ...
      .addAutoParam(Math.round(d1.position.y), PrimeColor.blue)
      .addAutoParam(Math.round(d2.position.x), PrimeColor.darkGreen)
      .addAutoParam(Math.round(d2.position.y), PrimeColor.darkGreen)
      .addAutoParam(Math.round(v_result.x), PrimeColor.raspberry)
      .addAutoParam(Math.round(v_result.y), PrimeColor.raspberry);

    const f2 = new Formula('v_1 + v_2 = v_3');
    return [f2, f1];
  });
</script>

<Canvas2D {draggables} {formulas} title="Vector addition">
  ...
  <Latex2D latex={'v_3'} position={v_result} color={PrimeColor.raspberry} />
