<script lang="ts">
  ...
  const mat1 = new Matrix(new Matrix2(0, 1, 1, 0), 'T_A');
  const mat2 = new Matrix(new Matrix2(2, 0, 0, 1), 'T_B');

  const controls = $derived.by(() => {
    return Controls.add(flipped ? mat2 : mat1)
      .addButton('<>', PrimeColor.yellow, () => (flipped = !flipped))
      .add(flipped ? mat1 : mat2);
  });

  const shadowM1 = $derived(mat1.disabled ? new Matrix2(1, 0, 0, 1) : controls[0]);
  const shadowM2 = $derived(mat2.disabled ? new Matrix2(1, 0, 0, 1) : controls[2]);
    ...
    if (!mat1.disabled && !mat2.disabled) {
      ...
      return [
        new Formula('T = \\$1 \\cdot \\$2')
          .addAutoParam(flipped ? 'T_B' : 'T_A')
          .addAutoParam(flipped ? 'T_A' : 'T_B')
      ];
    } else if (!mat1.disabled) {
      // If only controls[0] enabled
      return [new Formula('T = \\$1').addAutoParam(flipped ? 'T_B' : 'T_A')];
    } else if (!mat2.disabled) {
      // If only controls[2] enabled
      return [new Formula('T = \\$1').addAutoParam(flipped ? 'T_A' : 'T_B')];
    }
