<script lang="ts">
  ...
  // Number of controls before the per-distribution sliders start (here: just the N slider).
  const OFFSET = 1;
  ...
  const inDistrControls = Controls.addSlider(100, 0, 300, 1, PrimeColor.blue, { label: 'N' });
