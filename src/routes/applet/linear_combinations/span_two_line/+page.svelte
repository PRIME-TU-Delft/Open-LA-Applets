<script>
  ...
  let controls = Controls.addSlider(3, 1, 5, 0.5, PrimeColor.raspberry, 'length of v', (n) =>
    n.toFixed(1)
  ).addSlider(5, 1, 5, 0.5, PrimeColor.yellow, 'length of u', (n) => n.toFixed(1));
...
<Canvas3D bind:controls>
  ...
  {#if controls[0] * controls[1] > 0 && Math.abs(controls[0]) <= Math.abs(controls[1])}
    <Vector3D direction={v} length={controls[0]} color={PrimeColor.raspberry} alwaysOnTop />
  {:else}
