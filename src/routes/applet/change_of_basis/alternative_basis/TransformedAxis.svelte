...
<g>
  ...
  {#each axisIndeces as index, i (i)}
    ...
    {#if index != 0 && showAxisNumbers}
      ...
      {#if index > 0 && index % 2 == 0}
        <Latex2D latex={index.toLocaleString('en-GB')} position={new Vector2(-0.3, index + 0.12)} />
      {:else if index % 2 == 0}
        <Latex2D latex={index.toLocaleString('en-GB')} position={new Vector2(-0.55, index + 0.1)} />
      {/if}
    {/if}
  {/each}
