...
const transitionSteps: SlideShowSteps<S> = [
  (t, state) => {
    ...
    return { state, labelNext: 'R1 = R2 & R2 = R1', labelPrev: 'Original state' };
  },
  (t, state) => {
    // STEP 2
    // R2 = R2 - 2*R1
    ...
    return {
      state,
      labelNext: 'R2 = R2 - 2*R1 & R3 = R3 - 4*R1',
      labelPrev: 'R2 = R2 + 2*R1 & R3 = R3 + 4*R1'
    };
  },
  (t, state) => {
    // STEP 3
  ...
  (t, state) => {
    ...
    return { state, labelNext: 'R2 = R3 & R3 = R2', labelPrev: 'R2 = R2 & R3 = R3' };
  },
  (t, state) => {
    // STEP 5
    // R1 = R1 - 2*R2
    ...
    return {
      state,
      labelNext: 'R1 = R1 - 2*R2 & R3 = R3 + 5*R2',
      labelPrev: 'R1 = R1 + 2*R2 & R3 = R3 - 5*R2'
    };
  },
  (t, state) => {
    // STEP 6
