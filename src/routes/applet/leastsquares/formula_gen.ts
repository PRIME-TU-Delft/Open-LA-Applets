...
export function projectPoints(
    ...
    return ps.map((p) => {
        ...
        isLeastSquares ? (dist = dist * dist) : dist;
        return { p, pt, dist };
    });
}
...
export function calcTotalDist(u_ts: any[]) {
    return u_ts
        .map((p) => p.dist)
...
export function setFormulas(u_ts: any[], isOrthogonal: boolean, isLeastSquares: boolean) {
