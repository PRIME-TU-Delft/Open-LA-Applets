<script>
  ...
  // const u = new MathVector3(2, 3, 4); // Vector U - detached from the plane
  ...
  const v = $derived(u.clone().multiply(new MathVector3(1, 1, 0))); // direction of projection on plane
...
<Canvas3D {controls}>
  ...
  <!-- Plane p -->
  <PlaneFromNormal
    position={new MathVector3(0, 0, 0.01)}
  ...
  <Angle3D origin={u_proj} vs={[new MathVector3(0, 1, 0), u.clone().sub(u_proj)]} size={0.5} />
  <Angle3D origin={u_proj} vs={[new MathVector3(1, 0, 0), u.clone().sub(u_proj)]} size={0.5} />
  <Latex3D
