...
<Canvas3D cameraPosition={new Vector3(3.31, 6.55, 15.68)} zoom={38} bind:sliders>
  ...
  <Vector3D direction={u} color={PrimeColor.green} origin={v_0} length={ru_len} let:endPoint>
    <Latex3D position={endPoint} latex={`\\space r\\bold{u}`} offset={0.5} color={PrimeColor.green}/>

    <!-- Vector v -->
    <Vector3D direction={endPoint} color={PrimeColor.yellow} length={endPoint.length()} />
    <Latex3D position={endPoint.clone().add(new Vector3(0.4, 0, 0))} latex={`\\bold{v}`} offset={-2} color={PrimeColor.yellow}/>
  </Vector3D>
  ...
  <Vector3D
    ...
    radius={0.07}
  />
  <Latex3D position={v_0.clone().add(u)} latex={'\\bold{u}'} offset={0.4} color={PrimeColor.ultramarine}/>
