<script lang="ts">
  ...
  let origin = new Vector2(-1, 0);

  let w_plus_o = new Vector2(0, 3); //vectors are calculated this way because a draggable takes a global position, not a vector relative to origin

  $: w = w_plus_o.clone().sub(origin);

  let v_plus_o = new Vector2(1, -2);
...
<Canvas2D gridType={GridType.None} {formulas} cameraZoom={1.8}>
