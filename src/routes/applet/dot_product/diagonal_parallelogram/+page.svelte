<script lang="ts">
    ...
    $: isAlmostOrthogonal = Math.abs(w.clone().normalize().dot(v.clone().normalize())) <= 0.02 && !v.equals(w)
    ...
    $: snapline(v,orthV, w, orthW);
  </script>
  
  <Canvas2D gridType={GridType.None} {formulas}>
    <Vector2D direction={orthV} />
    <Vector2D direction={orthW}/>
