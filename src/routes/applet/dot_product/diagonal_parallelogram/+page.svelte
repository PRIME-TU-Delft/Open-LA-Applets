...
<Canvas2D >
  ...
  <!-- Non interactive part of image -->
      ...
      <!-- Bases -->
      <Vector2D origin={o_static} direction={v_static} length={v_static.length()} color={PrimeColor.ultramarine} let:endPoint>
        <Latex2D position={endPoint} latex={'v'} offset={new Vector2(-0.2, 0.5)} color={PrimeColor.ultramarine} />
      </Vector2D>
      <Vector2D origin={o_static} direction={w_static} length={w_static.length()} color={PrimeColor.green} let:endPoint>
        <Latex2D position={endPoint} latex={'w'} offset={new Vector2(-0.2, 0.2)} color={PrimeColor.green}/>
      </Vector2D>

      <!-- v+w -->
      <Vector2D origin={o_static} direction={vPlusW_static} length={vPlusW_static.length()} color={PrimeColor.red} let:endPoint>
        <Latex2D position={endPoint} latex={'v + w'} offset={new Vector2(0.2, 0.2)} color={PrimeColor.red} />
      </Vector2D>
