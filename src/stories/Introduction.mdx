...
split view? A [2D applets](http://localhost:6006/?path=/docs/initialize-canvas2d--docs) uses SVG and D3.js to render the
graphics, while a [3D applet](http://localhost:6006/?path=/docs/initialize-canvas3d--docs) uses WebGL via Threlte. Second,
do you want to use controls like sliders and toggles? If so, you can read up on using these controls in the
...
[Formula](?path=/docs/initialize-formula--docs) docs as a reference.
...
In the book of choice, you can embed the applet using an iframe. You can find more information on how to do this in the
