<script lang="ts" generics="State">
  ...
  let {
    ...
    onReset,
    hideButtons = false
  }: ControllerAndActivityPanelProps = $props();

  const firstSlider = controls.controls.findIndex((controller) => controller instanceof Slider);
  let sliderExpanded = $state(firstSlider);
  /**
