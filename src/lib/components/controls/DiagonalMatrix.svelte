  ...
  <div class="grid max-w-28 grid-cols-2 gap-2 text-sm" class:disabled={controller.disabled}>
    ...
    <input
      ...
      bind:value={
        () => controller.value.tr,
        (v) => {
          controller.value.tr = v;
          controller.value.bl = v; // Ensure diagonal elements are equal
        }
      }
    />
    <input
      ...
      bind:value={
        () => controller.value.bl,
        (v) => {
          controller.value.bl = v;
          controller.value.tr = v; // Ensure diagonal elements are equal
        }
      }
    />
    <input
