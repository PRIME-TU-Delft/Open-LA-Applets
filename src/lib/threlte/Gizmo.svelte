<script lang="ts">
	...
	useTask(
		() => {
			...
			renderer.setViewport(renderer.domElement.offsetWidth - size, 0, size, size);
	...
	$effect.pre(() => {
		...
		clickTarget.style.left = `${+boundingRect.left}px`;

		clickTarget.style.bottom = '';
		clickTarget.style.top = `${+boundingRect.top}px`;
...
<HierarchicalObject>
	<T is={root}>
		...
		>
			<T.SpriteMaterial
				map={getSpriteTexture(textureSize, PrimeColor.raspberry, 'Y')}
				opacity={p[0] >= 0 ? 1 : 0.5}
			/>
		</T.Sprite>
		...
		>
			<T.SpriteMaterial
				map={getSpriteTexture(textureSize, PrimeColor.raspberry)}
				opacity={p[0] >= 0 ? 0.5 : 1}
			/>
		</T.Sprite>
		...
		>
			<T.SpriteMaterial
				map={getSpriteTexture(textureSize, PrimeColor.darkGreen, 'Z')}
				opacity={p[1] >= 0 ? 1 : 0.5}
			/>
		</T.Sprite>
		...
		>
			<T.SpriteMaterial
				map={getSpriteTexture(textureSize, PrimeColor.darkGreen)}
				opacity={p[1] >= 0 ? 0.5 : 1}
			/>
		</T.Sprite>
		...
		>
			<T.SpriteMaterial
				map={getSpriteTexture(textureSize, PrimeColor.blue, 'X')}
				opacity={p[2] >= 0 ? 1 : 0.5}
			/>
		</T.Sprite>
		...
		>
			<T.SpriteMaterial
				map={getSpriteTexture(textureSize, PrimeColor.blue)}
				opacity={p[2] >= 0 ? 0.5 : 1}
			/>
		</T.Sprite>
	</T>
