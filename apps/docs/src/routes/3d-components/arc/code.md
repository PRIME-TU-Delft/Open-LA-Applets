...
<script lang="ts">
  ...
  //used as initial u for 180 degree angle
  let vtemp = new Vector3(1, 1, 0);

  //moving vector of 180 degree angle
  $: v2 = unitX
    .clone()
    .multiplyScalar(Math.cos(t))
    .add(vtemp.clone().multiplyScalar(Math.sin(t)));
</script>
```
...
<Canvas3D>
  ...
  <Arc3D points={[unitX, v2]} color={'black'} u={new Vector3(0,1,0)}
  origin={unitX.clone().multiplyScalar(-2)}/>
