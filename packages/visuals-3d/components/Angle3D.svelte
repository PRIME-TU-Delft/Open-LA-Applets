...
{#if (Math.abs(u1.dot(u2)) <= 0.005 && !u1.equals(u2) && !forceRoundAngle) || forceRightAngle }
	...
	<Line3D color={color} points={[u1.clone().add(origin), u1.clone().add(u2).add(origin)]}/>
	<Line3D color={color} points={[u2.clone().add(origin), u1.clone().add(u2).add(origin)]}/>

{:else if !vs[0].equals(vs[1])}

	<Arc3D points={[u1, u2]} color={'black'} origin={origin}  pointsOnArc={25}/>
